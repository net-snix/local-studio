 ...
 */
import { writeTranscriptSnapshot, TRANSCRIPT_CACHE_PREFIX } from "../src/features/agent/workspace/transcript-cache";
...
for (const [label, messages, blockChars] of [
  ...
  for (let session = 0; session < 24; session += 1) {
    ...
    counts.push([...storage._entries.keys()].filter((k) => k.startsWith(TRANSCRIPT_CACHE_PREFIX)).length);
  }

  // The written entry is what the cap produced, not the transcript we handed in.
  const writtenKb = Math.round(
    ([...storage._entries.values()][0]?.length ?? 0) / 1024,
  );
  const survivors = counts[counts.length - 1];
  const collapses = counts.filter((count, index) => index > 0 && count < counts[index - 1] - 1).length;
  console.log(
...
console.log("A survivor count of 1 means every write evicted the rest: the cache holds nothing when it is needed most.");
